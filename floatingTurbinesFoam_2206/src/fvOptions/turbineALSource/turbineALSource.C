...
void Foam::fv::turbineALSource::createOutputFile()
{
    ...
    *outputFile_<< "time,angle_deg,tsr,cp,cd,cq,ct,Fx,Fy,Fz";
...
void Foam::fv::turbineALSource::motionUpdate()
{
    // Should be defined for each turbine type
}


void Foam::fv::turbineALSource::motionInitialise()
{
    // Should be defined for each turbine type
}

void Foam::fv::turbineALSource::computeRigidBodyLoads(scalar rhoRef, vector moment)
{
    // Should be defined for each turbine type
}

void Foam::fv::turbineALSource::writeRigidBodyLoads(vector force, vector moment, vector refPoint)
{
    //if (debug)
    //{
        Info << "Writing loads to 'turbineSixDoFLoads' dict" << endl;
        Info << "Force: " << force << endl;
        Info << "Moment: " << moment << endl;
        Info << "Application point: " << refPoint << endl;
    //}
    ...
    if(time_.foundObject<IOdictionary>("turbineSixDoFLoads"))
    {
        ...
        updateDbDictionary.set("force", force);
        updateDbDictionary.set("moment", moment);
        updateDbDictionary.set("refPoint", refPoint);

        // Needed to update the IOdictionary
        const_cast<dictionary& > (loadsDict) = updateDbDictionary;
        
        Info << "Updating 'turbineSixDoFLoads' IOdictionary" << endl;
    }
}
...
void Foam::fv::turbineALSource::printPerf()
{
    ...
    Info<< "Rotor drag coefficient from " << name_ << ": " << dragCoefficient_
        << endl;
    Info<< "Rotor thrust coefficient from " << name_ << ": " << thrustCoefficient_
        << endl << endl;
}
...
bool Foam::fv::turbineALSource::read(const dictionary& dict)
{
    if (cellSetOption::read(dict))
    {
        ...
        // Read prescribed motion parameters if present
        ...
        coupleRigidBodyLoads_ = rigidBodyMotionDict_.lookupOrDefault("coupleLoads", false);
